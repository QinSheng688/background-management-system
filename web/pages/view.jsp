                    ...
                    <ul class="nav nav-drawer">
                        <c:forEach items="${menuList}" var="menu">
                            <li
                             ...
                             >
                                <a  href="javascript:void(0)"onclick="jump('${menu.href}','${menu.title}')"><i class="mdi mdi-menu"></i> ${menu.title}</a>
                                <c:if test="${not empty menu.list}">
                                    <ul class="nav nav-subnav">
                                        <c:forEach items="${menu.list}" var="menulist">
                                            <li
                                                    class="nav-item nav-item-has-subnav "
                                            >
                                                <a href="#!"onclick="jump('${menulist.href}','${menulist.title}')">${menulist.title}</a>
                                            </li>
                                        </c:forEach>
                                    </ul>
                                </c:if>
                        ...
                        <li class="dropdown dropdown-profile">
                            <a href="javascript:void(0)" data-toggle="dropdown">
                                <img class="img-avatar img-avatar-48 m-r-10"
                                     src="${ctx}/static/images/sheng.jpg" alt="后台系统"/>
        ...
        $.ajax({
            ...
            success(res) {
                if (res.code==200){
                    ...
                    let pids = document.getElementsByName("pids");
                    for (let i = 0; i < pids.length; i++) {
                        if(pids[i].value==res.data.pId){
                            pids[i].selected=true;break;
                        }
            ...
            $.ajax({
                ...
                success(res) {
                    if (res.code==200){
                        ...
                        document.getElementById("password").value="保密";
...
<%--    商家js--%>
    <script>
            ...
            $.ajax({
                ...
                success(res) {
                    if (res.code==200){
                        ...
                        document.getElementById("busName").value=res.data.busName;
                        document.getElementById("tel").value=res.data.tel;
                        document.getElementById("address").value=res.data.address;
        ...
        function enableBus(s){
            var url;
            if (s<2){
                url="${ctx}/api/bus/enableBuses";
            }else {
                url="${ctx}/api/bus/deleteBuses";
            }
